...
Обычное сообщение целиком едет внутри пуша: шифротекст «привет» — это десятки
байт при лимите APNs в 4 КБ, так что расширению нечего дозапрашивать, оно
расшифровывает и пишет. Отдельный флоу нужен только для того, что в лимит не
влезло: длинный текст или конверт на много устройств. Тогда в пуше едет флаг
«дотянуть с сервера», расширение забирает тело запросом, а если не успевает за
свой бюджет — показывает баннер и оставляет догрузку приложению. Побочно решается
