...
Сеть в момент доставки есть по определению — пуш пришёл. Смысл в другом: у
расширения тридцать секунд, и дозапрос к серверу за телом сообщения это лишняя
задержка и лишняя точка отказа на медленной сети. Поэтому шифротекст едет внутри
полезной нагрузки APNs: один пуш — полное сообщение. Конверт помещается в лимит
4 КБ и для личного чата, и для группы с sender key; если не влез (крупный
конверт на много устройств), ставится флаг «дотянуть с сервера». Побочно решается
порядок: расширение видит seq и помечает разрыв для догрузки.
