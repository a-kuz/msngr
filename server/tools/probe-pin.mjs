...
const t0 = Date.now();
const pinRes = await api(`/api/chats/${chat.chatId}/pin-message`, { token: a.token, body: { seq: sent.seq } });
...
console.log("actor got chat/pinned:", !!fa, fa ? `${fa.at - t0}ms pinnedSeq=${fa.state?.pinnedSeq}` : "");
console.log("peer  got chat/pinned:", !!fb, fb ? `${fb.at - t0}ms pinnedSeq=${fb.state?.pinnedSeq}` : "");
