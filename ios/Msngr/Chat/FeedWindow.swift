...
///
/// Пока пользователь смотрит историю, граница стоит на месте: ничто из
/// прочитанного не уезжает. Пока лента у низа, окно скользит — граница
/// пересчитывается по вместимости, и приходящие сообщения выталкивают из окна
/// самые старые. Без этого потолка окно растёт всё время, что открыт чат, и
/// каждая вставка перечитывает, декодирует и диффит всё, что в нём накопилось.
