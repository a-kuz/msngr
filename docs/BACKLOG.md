...
| B5 | `this.blockers` caches another object's block list in a field until eviction | do D5, ev T20 | agent-007 | 4baac9d · the field and the invalidation call are gone, the chat asks `blockPair` on every check; the copy in the chat's own storage fanned out on change is B26 (it changes what the object stores, so it comes with the stand wipe) — smoke 490 ok, 25 block checks green, on a private stand |
