...
`fcb3712` (pinned bar drawn from its own row): the tap on the bar reaching a
...
the working tree after merging main (`2fcc006`): the fanout rework landed
...
`ChatViewModel.observePinnedMessage` (commit `fcb3712`) reads the pinned row by
