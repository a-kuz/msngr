...
nothing here comes from the sender's own cache.
...
at 0.335, 0.471, 0.602, 0.737, 0.870, with the transport reading `0:02
elapsed` / `−0:00 remaining`. The 2026-08-13 run's symptom was the opposite:
a black frame, a crossed-out play glyph, and `--:-- … --:--`, meaning the
AVPlayerItem never loaded at all. The sender side plays too: the same video
opened on A reports duration 0:02 and reaches position 1.
...
Also corrected a comment on `VideoPlayerPage` that claimed range-request
streaming. It plays the decrypted file from the media cache; there is no
streaming (see below).

## Blurhash placeholder — observed, with a caveat about the stand

Captured by an A/B of the same sequence. Media cache cleared through Settings
("Очистить кэш медиа", 938 KB → 0 files on disk), app killed so the in-memory
`ImagePipeline` cache goes with it, then screen-recorded: launch, tap the chat,
watch the feed come in. Then the identical sequence again with the cache now
warm.

Cold run, 0.47 s after the album tiles start sliding in: every tile is a smooth
two-tone gradient with a diffuse light blob where the numeral is — the 32×32
blurhash decode, upscaled. Numerals unreadable. 0.9 s later the same tiles show
readable digits. Warm run, at the matching frame of the same animation: the
digits are already readable — the real photo is drawn the first time the tile
appears. Measured over the feed area, the fraction of strong-edge pixels
(|Laplacian| > 40) in the cold run trails the warm run by about 0.4 s and the
two converge ~1.5 s after the chat opens.

Caveat worth carrying forward: on localhost the placeholder is only on screen
for a few frames. A second attempt — scroll into a region of the chat that had
never been downloaded, then hold still — produced nothing to measure: by the
time the flick's deceleration ended (frame-to-frame difference at zero from
4.9 s onward) the photos had already loaded, and the feed's sharpness sat flat
at 1.115 for the following 2.5 s. The placeholder is real and the code path
runs; its duration on this stand is not representative of a phone on a real
network. Seeded photos are flat colour and compress hard — the whole chat's
media was 938 KB — which makes the window shorter still.
...
B (incoming) time only. No per-tile status anywhere. Same for the single photo
and the video bubble.
...
`AVAssetResourceLoaderDelegate` feeding AVPlayer decrypted ranges. Noted in the
...
composer are normal. It survived a scroll and two screenshots roughly ten
seconds apart, so it is not a one-frame artefact of the crossfade. Leaving the
chat and re-entering renders it correctly, and it never came back. Not in the
media path; not investigated further. `ChatScreen` swaps `requestCard` for
`messagesList` inside `withAnimation(Theme.spring)`, which is where I would
look first.
...
disappears into the image. Every other overlay on media — the status capsule —
has a dark backdrop.
...
as "ьувшффдшсу". Two ways out: tap the globe key once when the software
keyboard is up (fixes it for the rest of the session), or push text through
`xcrun simctl pbcopy` and the Paste menu item. Writing `AppleKeyboards` into
the device's Preferences domain and rebooting did not help.
