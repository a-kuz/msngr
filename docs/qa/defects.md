...
### A call card's title once drew over its icon after an edit
Seen 2026-09-02 in the group-calls run (`runs/2026-09-02-group-calls.md`):
in Standup on alfa, the second of three call cards — its text edited several
times as members came and went and the call ended — showed its title
overlapping the icon area and clipped («Гр…»), while the cards above and
below read fine. Reproduced once; a scroll away and back did not redraw it
during the run. Most likely the row's reconfigure-in-place after an edit
...
took two extra legs, so with four people in the call the add button still
opened the picker and the pick was dropped without a word. Closed by the
room-based group call (b607a3e): the invite has no client-side cap, the SFU
is the ceiling.
