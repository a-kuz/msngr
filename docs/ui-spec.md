...
- Кнопка «вниз»: появляется при `contentOffset.y > 300`, переход
  `scale + opacity` пружиной `Theme.springFast`.
...
Заявка на переписку — панель «… хочет вам написать» с кнопками «Принять» и
«Заблокировать». Смена ключа собеседника — баннер «Код безопасности изменился»
...
Приоритет превью: печатает → «Черновик: …» → последнее сообщение
(«Сообщение удалено», «Фото», «Видео», «Голосовое сообщение», имя файла,
«Альбом», иначе текст) → «Заявка на переписку» для пустой заявки. Медиа-превью
рисуются иконкой SF Symbols в цвете акцента; системные сообщения в превью не
попадают.
...
Поиск по списку локальный: по заголовку чата и юзернейму собеседника среди
активных и архивных. Поиска по тексту сообщений в интерфейсе нет.
...
- Бейдж — сумма `unreadCount` по чатам из локальной БД; при обнулении
  непрочитанных доставленные уведомления этого чата снимаются из шторки.
